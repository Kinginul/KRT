...
local function ensureTrollGui()
    if trollGuiRef and trollGuiRef.Parent then
        return trollGuiRef
    end

    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "KinginulTrollGui"
    screenGui.ResetOnSpawn = false
    screenGui.Parent = CoreGui
    trollGuiRef = screenGui
    return screenGui
end

local function showPrankText(text, color, duration)
    local gui = ensureTrollGui()
    local label = Instance.new("TextLabel")
    label.Size = UDim2.new(0, 360, 0, 60)
    label.Position = UDim2.new(0.5, -180, 0.5, -30)
    ...
    TweenService:Create(label, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        Position = UDim2.new(0.5, -180, 0.42, -30),
    ...
    task.delay(duration or 1.3, function()
        if label and label.Parent then
            TweenService:Create(label, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
                Position = UDim2.new(0.5, -180, 0.3, -30),
                TextTransparency = 1,
            }):Play()
            task.delay(0.2, function()
                if label and label.Parent then
...
local function toggleTrollMode()
    ...
    if trollState.enabled then
        ...
        showPrankText("SPIN MODE", Color3.fromRGB(255, 92, 160), 1.3)
...
local function prankLaughBurst()
    local char = player.Character
    if char then
        playEmote(char, "rbxassetid://14548619594")
    end
    showPrankText("HAHA! PRANKED!", Color3.fromRGB(255, 196, 77), 1.6)
    showToast("Laugh Burst")
end

local function prankScreenShake()
    ...
    local connection = RunService.RenderStepped:Connect(function()
        if os.clock() - start > 1.4 then
            connection:Disconnect()
            return
        end

        cam.CFrame = cam.CFrame * CFrame.Angles(math.rad(math.random(-8, 8)), math.rad(math.random(-8, 8)), math.rad(math.random(-8, 8)))
    end)
    ...
    showPrankText("SHAKE!", Color3.fromRGB(127, 255, 212), 1.2)
    showToast("Screen shake")
end

local function prankFakeBan()
    local gui = ensureTrollGui()
    ...
    title.Text = "YOU GOT PRANKED"
    title.TextColor3 = Color3.fromRGB(255, 82, 94)
    ...
    sub.Position = UDim2.new(0.15, 0, 0.55, 0)
    sub.BackgroundTransparency = 1
    sub.Text = "lol, noob mode activated"
    ...
    showToast("Fake ban triggered")
end

local function prankSpinChaos()
    ...
    local connection = RunService.RenderStepped:Connect(function()
        if player.Character then
            local currentRoot = player.Character:FindFirstChild("HumanoidRootPart")
            if currentRoot then
                currentRoot.CFrame = currentRoot.CFrame * CFrame.Angles(0, math.rad(24), 0)
            end
        end
    end)
    ...
    showPrankText("CHAOS SPIN", Color3.fromRGB(255, 102, 204), 1.6)
    showToast("Chaos spin")
end

local function prankNoobFloat()
    local char = player.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if not root then
        return
    ...
    bodyVelocity.Velocity = Vector3.new(0, 18, 0)
    ...
    local connection = RunService.RenderStepped:Connect(function()
        if player.Character then
            local currentRoot = player.Character:FindFirstChild("HumanoidRootPart")
            if currentRoot then
                currentRoot.CFrame = currentRoot.CFrame * CFrame.Angles(0, math.rad(14), 0)
            end
        end
    end)
    ...
    showPrankText("NOOB FLOAT", Color3.fromRGB(138, 201, 255), 1.4)
    showToast("Float prank")
end
...
local function createUI()
    ...
    local laughBtn = createStyledButton("Laugh Burst", 6, visualContent, function()
        prankLaughBurst()
    end, "L")

    local shakeBtn = createStyledButton("Screen Shake", 7, visualContent, function()
        prankScreenShake()
    end, "S")

    local fakeBanBtn = createStyledButton("Fake Ban", 8, visualContent, function()
        prankFakeBan()
    end, "B")

    local chaosBtn = createStyledButton("Chaos Spin", 9, visualContent, function()
        prankSpinChaos()
    end, "C")

    local noobFloatBtn = createStyledButton("Noob Float", 10, visualContent, function()
        prankNoobFloat()
    end, "N")
